...
void setup() {
  ...
  ADCWindowBegin(DISABLE, 1023, 190); //Do not use the Arduino analog functions until you call ADCWindowEnd()
  startTimer(44100);
  disableTimer();
  Serial.begin(9600);
}
...
void loop() {
  ...
  //Serial.println();
  if (buttonPressed(2) && TOGGLE==0)  {
      enableTimer();
      TOGGLE=1;
      digitalWrite(13, HIGH);
    }
  else 
  if (buttonPressed(2) && TOGGLE==1) 
    {
     disableTimer();
     TOGGLE=0;
     digitalWrite(13, LOW);
