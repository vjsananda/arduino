...
int sendPostRequest(String body) {
  ...
  while (client.available() == 0) {
    if (millis() - timeout > 10000) {
      ...
      client.stop();
      updateSleepTime(SLEEP_TIME_IF_ALARM_ERROR);
      return 0;
    }
  }
  ...
  JsonObject & root = jsonBuffer.parseObject(jsonResponse);
   ...
   if (!root.success())
   {
    ...
    //If error count crosses a threshold, then multiply retry actions by a scaling factor
     if (pendingAction == ALARM_ACTION ) {
       updateSleepTime(SLEEP_TIME_IF_ALARM_ERROR);
     }
     else {
       updateSleepTime(SLEEP_TIME_IF_PING_ERROR);
     }  
     return 0;
   }
   ...
   if (e == 0) {
     ...
     updateSleepTime( sleepTimeAfterError( rtcGetState() ) );
   }
   return e;
}
