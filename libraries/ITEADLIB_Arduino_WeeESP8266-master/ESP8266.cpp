...
ESP8266::ESP8266(SoftwareSerial &uart, HardwareSerial &dbg): m_puart(&uart),m_dbg(&dbg)
{
    m_onData = NULL;
    m_onDataPtr = NULL;
}
...
void ESP8266::recvEcho(uint32_t timeout) {
    char a ;
    unsigned long wdog_timer = 0;
    unsigned long start = millis();
    while ( wdog_timer < timeout) {
        wdog_timer = millis() - start ;
...
void ESP8266::recvRaw(char * buffer, unsigned long len, char start_char, uint32_t timeout) 
{
   unsigned long idx ;
   idx = 0;
   char a ;
   char start_char_detected = 0;
    unsigned long start = millis();
    while (millis() - start < timeout) {
        while( m_puart->available() > 0 && idx < len) {
        a = m_puart->read();
        if ( a == start_char ) start_char_detected = 1 ;
        if ( start_char_detected)
            buffer[idx++] = a;
        }
    }
}
